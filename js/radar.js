...
function buildRubberHeaderHtml(rubber, panelIndex, dashed) {
    ...
    const ytResult = getRubberYouTubeVideoId(rubber);
    const youtubeVideoId = ytResult?.videoId || null;
    const ytIsFallback = ytResult?.isFallback || false;
    ...
    const ytEnBadge = ytIsFallback ? `<span class="yt-en-badge" aria-hidden="true">EN</span>` : '';
    const rubberImgHtml = youtubeVideoId
        ? `<div class="radar-rubber-img-wrap">` +
            `<a href="#" class="radar-rubber-img-link" data-yt-videoid="${youtubeVideoId}" title="${safeVideoLabel}" aria-label="${safeVideoLabel}">` +
                ...
                `<span class="radar-rubber-yt-badge" aria-hidden="true"><img src="/images/youtube.ico" alt="">${ytEnBadge}</span>` +
            `</a>` +
            `<div class="radar-rubber-img-placeholder" style="display:none;"></div>` +
        `</div>`
        : `<div class="radar-rubber-img-wrap">` +
    ...
    return `
        <div class="radar-comparison-header-side${panelIndex === 1 ? ' radar-comparison-header-side--right' : ''}">
            ...
            <div class="radar-info-name-row">
                ...
                <button class="radar-pin-btn${isPinned ? ' radar-pin-btn--active' : ''}" data-panel-index="${panelIndex}" data-hint="${isPinned ? 'Pinned' : 'Unpinned'}">${pinIcon}</button>
            </div>
            ${rubberImgHtml}
...
function buildRadarComparisonHtml(first, second) {
    ...
    const playersRowHtml = `
        <div class="radar-cmp-cell radar-cmp-cell--left radar-cmp-cell--players">${buildPlayersColumnHtml(first, 'right', { gifTracker })}</div>
        <div class="radar-cmp-cell radar-cmp-cell--label">${tUi('PLAYERS')}</div>
        <div class="radar-cmp-cell radar-cmp-cell--right radar-cmp-cell--players">${buildPlayersColumnHtml(second, 'left', { gifTracker })}</div>
    `;
